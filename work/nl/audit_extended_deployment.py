...
hist = np.load(OUT / "hist2_378.npy").astype(np.float64)
stack = np.load(OUT / "oof378_ridge.npy").astype(np.float64)
unique = json.loads((OUT / "hist2_unique.json").read_text())["kept_indices"]
ranking = np.mean(
    [np.abs(np.load(OUT / f"state2_{f}_beta.npy")) for f in RANK_FOLDS], axis=0
)
...
report = {
    "tag": "154_extended_pool_deployment",
...
(OUT / "154_extended_pool_deployment.json").write_text(json.dumps(report, indent=2) + "\n")

...
names = json.loads((OUT / "hist2_keys.json").read_text())
hist = np.load(OUT / "hist2_408.npy").astype(np.float64)
...
(OUT / "hist2_unique.json").write_text(json.dumps(report, indent=2) + "\n")

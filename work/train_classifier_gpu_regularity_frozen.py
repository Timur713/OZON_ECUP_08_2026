...
def event_regularity_profile(raw):
    """Return bounded recent-event rhythm features per raw channel.

    The first two values exactly preserve ``event_cadence``.  Four additional
    values describe the last four observed intervals without reading labels:
    their mean, coefficient of variation, current overdue phase, and the last
    gap relative to the recent mean.  Missing rhythm maps to the conservative
    1.0 sentinel used by the original cadence branch.
    """
    ...
    if latest.shape[-1] < 2:
        ones = torch.ones_like(recency)
        return recency, last_gap, ones, ones, ones, ones

    gaps = latest[..., :-1] - latest[..., 1:]
    valid = (latest[..., :-1] >= 0) & (latest[..., 1:] >= 0)
    valid_float = valid.to(raw.dtype)
    count = valid_float.sum(-1)
    safe_count = count.clamp_min(1.0)
    gap_sum = (gaps * valid_float).sum(-1)
    mean_gap_raw = gap_sum / safe_count
    has_rhythm = count > 0
    mean_gap = torch.where(has_rhythm, mean_gap_raw, torch.ones_like(mean_gap_raw))

    centered = (gaps - mean_gap_raw[..., None]) * valid_float
    gap_variance = centered.square().sum(-1) / safe_count
    resolution = 1.0 / max(1, raw.shape[-1] - 1)
    gap_cv = torch.sqrt(gap_variance.clamp_min(0.0)) / (mean_gap_raw + resolution)
    gap_cv = torch.where(has_rhythm, gap_cv.clamp(0.0, 4.0) / 4.0,
                         torch.ones_like(gap_cv))
    overdue_phase = torch.where(
        has_rhythm,
        (recency / (mean_gap_raw + resolution)).clamp(0.0, 4.0) / 4.0,
        torch.ones_like(recency),
    )
    last_to_mean = torch.where(
        has_rhythm,
        (last_gap / (mean_gap_raw + resolution)).clamp(0.0, 4.0) / 4.0,
        torch.ones_like(last_gap),
    )
    return recency, last_gap, mean_gap, gap_cv, overdue_phase, last_to_mean
...
class Network(nn.Module):
    def __init__(
        ...
        event_summary=False, event_profile=False,
    ):
        super().__init__()
        ...
        event_width = (
            summary_channels * 6 if event_profile else
            summary_channels * 2 if event_summary else 0
        )
    ...
    def forward(self, x, static=None):
        ...
        if self.event_profile:
            raw = x[:, :self.summary_channels]
            pooled_parts.extend(event_regularity_profile(raw))
        elif self.event_summary:
            raw = x[:, :self.summary_channels]
...
def arguments():
    ...
    parser.add_argument(
        "--event-profile", action="store_true",
        help=(
            "append cadence plus recent mean-gap, variability, overdue phase, "
            "and last-gap-to-mean features"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--require-30-target", action="store_true",
        help="train only on anchors with an observed 30-day regression target",
    )
    parser.add_argument(
        "--head-selection", choices=("auto", "direct", "hurdle"), default="auto",
        help="restrict validation head selection instead of searching the mix grid",
    )
    parser.add_argument(
...
def main():
    ...
    if args.event_summary and args.event_profile:
        raise ValueError("--event-summary and --event-profile are mutually exclusive")
    ...
    if latest_anchor not in anchors:
        ...
        anchors.sort()
    if args.require_30_target:
        anchors = [anchor for anchor in anchors if anchor + 30 <= cutoff]
        if not anchors:
            raise ValueError("no anchors have an observed 30-day target")
    ...
    network = Network(
        ...
        args.window,
        summary_channels=(
            nchannels if args.summary or args.event_summary or args.event_profile else 0
        ),
        static_features=(
    ...
    print(
        ...
        f"event_profile={args.event_profile} "
        f"require_30_target={args.require_30_target} "
        f"head_selection={args.head_selection} "
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.mode == "validate":
            ...
            if args.head_selection == "direct":
                hurdle_weights = (0.0,)
            elif args.head_selection == "hurdle":
                hurdle_weights = (1.0,)
            else:
                hurdle_weights = np.linspace(0.0, 1.0, 21)
            mix_rows = []
            for hurdle_weight in hurdle_weights:

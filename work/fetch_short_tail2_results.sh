#!/usr/bin/env bash
# Fetch the second short queue, skipping intentionally stopped full runs.
set -u
...
LOG=/home/ubuntu/ecup/logs/short_tail2.log

wait_and_fetch() {
  local tag=$1 remote_tag=${tag}_full
  while ! "${SSH[@]}" \
      "test -s /home/ubuntu/ecup/work/${remote_tag}_final.npy" 2>/dev/null; do
    if "${SSH[@]}" "grep -q '^FAILED ${remote_tag}$' '$LOG'" 2>/dev/null; then
      date '+%Y-%m-%d %H:%M:%S %Z'
...
for tag in cls120hybmktclass cls90hyb cls60hyb cls240hybmkt cls180hyb; do
  wait_and_fetch "$tag"
done

echo SHORT_TAIL2_FETCH_DONE

...
ROOT = os.environ.get('ECUP_ROOT', '/Users/timur/Desktop/dev/OZON_ECUP_2026_3').rstrip('/')
MD = os.environ.get('ECUP_MAT', ROOT + '/work/mat').rstrip('/') + '/'
D0=date(2025,1,1)
MATS=['gmv','to_ord','to_cart','searches','active','gmv_cat','gmv_search','search','cat',
      'search_to_ord','cat_to_ord']
# never-used columns: daily funnel-completion flags + cart splits (reduced feature set)
MATS2=['has_search_to_ord','has_search_to_cart','has_cat_to_ord','has_cat_to_cart',
       'search_to_cart','cat_to_cart']
WIN2=[30,90,365]; DWIN2=[30,90,365]
...
def build(anchors, verbose=False):
    NU=int(os.environ.get('ECUP_NUSERS','250000')); A=len(anchors); cols={}; ar=None
    dow=np.array([(D0+timedelta(days=int(i))).weekday() for i in range(409)])
    wknd=(dow>=5).astype(np.float32)
    for m in MATS:
        M=np.load(MD+m+'.npy',mmap_mode='r')[:NU]; ND=M.shape[1]
    ...
    for m in MATS2:
        ...
        L1=np.maximum.accumulate(np.where(M>0,ar,np.int16(-1)),axis=1)
        a=np.empty((A,NU),dtype=np.float32)
        for ai,t in enumerate(anchors): a[ai]=t-L1[:,t]
        cols[f'{m}_rec']=a
        del M,cs,nz,csn,L1; gc.collect()
        if verbose: print("  +",m,flush=True)
    act=np.load(MD+'active.npy',mmap_mode='r')[:NU]
...
def targets(anchors,H=30):
    NU=int(os.environ.get('ECUP_NUSERS','250000'))
    gmv=np.load(MD+'gmv.npy',mmap_mode='r')[:NU]; ND=gmv.shape[1]

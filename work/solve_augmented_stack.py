...
if not args.dry_run:
    ...
    output = output_name if os.path.isabs(output_name) else ROOT + "submissions/" + output_name
    metadata_stem = os.path.splitext(os.path.basename(output_name))[0]
    if args.metadata_output:
        metadata_output = args.metadata_output
    elif os.path.isabs(output_name):
        metadata_output = os.path.join(
            os.path.dirname(output_name), f"{metadata_stem}_meta.json"
        )
    else:

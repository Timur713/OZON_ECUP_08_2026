...
WIN=[3,7,14,30,60,90,180,270,365]
DWIN=[7,30,90,180,365]

def build(anchors, verbose=False):
    NU=250000; A=len(anchors); cols={}
    ar=None
    for m in MATS:
        ...
        for W in WIN:
            ...
            for ai,t in enumerate(anchors): a[ai]=cs[:,t+1]-cs[:,max(0,t-W+1)]
            cols[f'{m}_s{W}']=a
        # same-window-last-year (target-aligned): [t+1-365, t+30-365]
        ...
        for W in DWIN:
            a=np.empty((A,NU),dtype=np.float32)
            for ai,t in enumerate(anchors): a[ai]=csn[:,t+1]-csn[:,max(0,t-W+1)]
            cols[f'{m}_d{W}']=a
        ...
        for ai,t in enumerate(anchors):
            ...
            g=(l1-l2).astype(np.float32); g[l1<0]=np.nan; g[l2<0]=np.nan
            gap[ai]=g
    ...
    R('gmv_LY','gmv_s30','r_LY_30')
    # "overdue": recency relative to own purchase rate
    for W in [90,180,365]:
        cols[f'overdue{W}']=(cols['gmv_rec']*cols[f'gmv_d{W}']/W).astype(np.float32)
    ...
    for j,n in enumerate(names):
        if ('_s' in n or '_LY' in n) and not n.startswith(('r_','sh_','conv','aov')):
